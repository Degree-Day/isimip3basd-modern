...
Fire Weather Index (FWI) and its five component indices. These daily fields are
the inputs ultimately passed to the wildfire (WF) model. Annual summaries are
derived separately for diagnostic and quality-control purposes.
...
The daily CFFWIS fields are the production interface to the WF model. Four
annual indicators are also derived from the daily xclim output for diagnostic
purposes only:

\begin{table}[htbp]
\centering
\caption{Annual FWI indicators retained for diagnostics and quality control.}
\label{tab:fwi-diagnostics}
\small
\begin{tabularx}{\textwidth}{lX}
\toprule
Indicator & Definition \\
\midrule
\texttt{fwixx} & Annual maximum FWI \\
\texttt{fwixd} & Annual count of days above the local historical
95th-percentile FWI threshold \\
\texttt{fwils} & Annual count of days above the local historical FWI midrange \\
\texttt{fwisa} & Annual maximum 90-day mean FWI \\
\bottomrule
\end{tabularx}
\end{table}
...
Abatzoglou et al. \cite{abatzoglou2019}. They support spatial and temporal QC,
model comparison, and interpretation, but they are not substituted for the
daily FWI sequence in the WF model.
...
\caption{Diagnostic example of daily FWI sequences for the Los Angeles--San Fernando study
