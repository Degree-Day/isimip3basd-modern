...
def test_fwi_rejects_obsolete_hurs_adjustment(tmp_path):
    ...
    humidity.attrs["bias_adjustment_preset_revision"] = 4
    humidity.to_dataset(name="hurs").to_zarr(path, mode="w", zarr_format=3)
    assert FWI.require_current_hurs_adjustment(tmp_path, "global") == 4
...
def test_dc_packing_covers_century_scale_arid_accumulation():
    dataset = xr.Dataset(
        {
    ...
    dataset["dc"][:] = 283_116.6

    packed = FWI.pack_indices(dataset)
    spec = FWI.PACKING_SPECS["dc"]
    decoded = packed["dc"].astype("float64") * spec.scale_factor + spec.add_offset
